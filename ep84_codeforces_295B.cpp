...
vector<ll> ans;
for (int k = 0; k < n; ++k)//k=0 mean (k=1 level) ,due to indexing of del_order we use k=0
{
	
	int k_v=del_order[k];// means del_order[k] is included
	loop1n(n)
	{
		for (int j = 1; j <=n; ++j)
		{
	...
	for (int i = 0; i <=k; ++i)
	{
		for (int j = 0; j <=k; ++j)
		{
			sum+=dist[del_order[i]][del_order[j]];//finding sum of all verices which are present now(after insertion)
		}
	}
	ans.pb(sum);
}
