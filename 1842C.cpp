...
/*----------------------code start here -----------------------------*/
// ll f(ll i,vl &v,vl &dp,map<ll,vl> &m)
// {
...
//     ll take=0;
//     ll nottake=f(i+1,v,dp,m);
//     debug(i)

//     if(m[v[i]].size()!=1)
//     {
//         ll pos=lower_bound(all(m[v[i]]),i)-m[v[i]].begin();
//         debug(pos)

//         // if(pos+1<m[v[i]].size())
//         // {
//         //     take=max(take,f(m[v[i]][pos+1]+1,v,dp,m)+(m[v[i]][pos+1]-i+1));
//         // }
//         fl(j,pos+1,m[v[i]].size())
//         {debug("YES",j)
//             take=max(take,f(m[v[i]][j]+1,v,dp,m)+(m[v[i]][j]-i+1));
//         }
//     }
