...
vvl cc;//connected component
vl currentcc;//current connected component

vl g[N];
bool vis[N];
void dfs(ll vertex)
{
	currentcc.pb(vertex);//take action on vertex before entering the vertex
	vis[vertex]=true;
...
loop(e)//as we each connection at a time .Therefore number of edges
{
	ll x,y;
	cin>>x>>y;
	g[x].pb(y);
...
{
	if(vis[i]) continue;
	currentcc.clear();//as it means we move to next component
	dfs(i);
	cc.pb(currentcc);
	count++;// means dfs on that component is completed
