...
in(n);
vl l,c;
set<ll> r;// so that automatically will be sorted(save O(n)-->time take)
vin(l,n);
loop(n)
{
    ll x;
...
debug(l)
vl ar(n);
loopn1(n)//loop from back so that it can see thr back element
//if loop(n) then its lower bound will be r[i] as r[i]>=l[i]
//so it canot see back as alredy taken
{
    debug(r)
