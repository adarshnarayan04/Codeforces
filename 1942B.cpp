...
void themagician(){
    ...
    vin(a,n);
    vl p(n);
    ll mex=n;//as last will have mex always n
    for (int i = n-1; i >=0; i--)
    {
      p[i]=mex-a[i];
      mex=min(mex,p[i]);
      // now left n-2 numbers are unknown nut we know p[n-1] is x
      // so mex upto n-2 is x now we know p[n-2] by this let y
      // so mex upto n-3 be min(x,y) as this twwo number is not present (but we minimum excluded number)
    }
    cout<<p;
