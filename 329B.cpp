...
vector<pair<int,int>>movements={
	...
	{0,1},
	{0,-1}
};//storing movents of knight possible from (0,0) so that can easily add to generate all moves
...
int bfs(string source,string dest)
{
	...
	lev[sourcex][sourcey]=0;//as intially taken as INF
	while(!q.empty())
	{
		...
		int y=v.second;
		q.pop();
		for(auto movement:movements)//like accessing the child in graph(here we are accesing the coordinates that knight can go in one move)
		{
		...
		if(lev[destx][desty]!=INF)//means not equals to intial value(INF)
			//taken INF as intal value not zero as lev of des can be zero in a case wehn source=dest
			//so we can say that lev!=0 then break
		{
			break;//as no longer to run bfs we have find the destination
		}
