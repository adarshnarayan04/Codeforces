...
/*----------------------code start here -----------------------------*/
//01 bfs
ll bfs(ll source, ll n,vector<pair<ll, ll>> g[])
{
    vector<bool>vis(n+1,0);
    vector<ll>lev(n+1,INF);//distance of all node source will be stored
    deque<ll>q;
    q.pb(source);//{wt,node} set sort will sort the pair(so first will be sorted in basis of 1st element then 2nd is value is 1st element is same)
    lev[source]=0;
    ...
    while(!q.empty())
    {
        ...
        for(auto &child:g[cur_v])
        {
            ...
            if(lev[cur_v]+wt<lev[child_v])
            {
                ...
                {
                    q.push_front(child_v);//same level
                }
            }
        }
    }
...
loop1(n+1)
{
    ...
    if(s[i-1]=='L')//as 1 based indexing
    {
        if(x!=0)
        g[i].pb({x,0});
        if(y!=0)
