...
void themagician(){
    ...
    cin>>a>>b;
    bool isswap=false;
    //making a as largest then reducing (the back digits but it remain largest)
    //as sum of a+b is constant 
    //there  mult is max when both number are equal
    //so we reduce the largest number and incraese the smallest number (so that there mult increases)
    loop(a.size())
    {
        if(!isswap)
        {
