...
void themagician(){
    ...
    ll sum=accumulate(all(dif),0LL);

    ll rem=k-1;//we use 1 cm tape for all k-1(largest element)
    ll ans=k-1;
    loop(rem)
    {
        sum-=(dif[i]);
    ...
    ans+=(sum+1);//we use single tap for left one after taking k-1 (we take +1  to cover 1->2 it require 2 cm but diff is 1)
