...
if(tot>=req)
{
	...
	if(numtr>=n)
	{
		...
		loop(normal.size())
		{
			cout<<normal[i]<<" "<<trum[i]<<nl;
			pos=i;
		}
		for (int i = pos+1; i < trum.size(); i+=2)
		{
			cout<<trum[i]<<" "<<trum[i+1]<<nl;
		}
	...
	else
	{
		...
		for(auto &x:m)
		{
			if(x.se.size()>=2&&x.fi!=trump)
			{
				sort(all(x.se));
				ll siz=x.se.size()/2;
				for (int i =x.se.size()-1; i>=1; i-=2)
				{
					cout<<x.se[i-1]<<x.fi<<" "<<x.se[i]<<x.fi<<nl;
					x.se.pop_back();
					x.se.pop_back();
				}
				loop(x.se.size())
			...
			else if(x.se.size()<2&&x.fi!=trump)
			{
				loop(x.se.size())
				{
					string z;
