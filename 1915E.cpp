...
/*----------------------code start here -----------------------------*/
//approach- for l to r sum to be equal odd(from l to r)=even (from l to r) //using prefix to store odd and even
// so equation become even[r]-even[l-1]=odd[r]-odd[l-1] (we have optimized the sum so that dont depend in l,r to be even or odd)
//rearring the equation (as done is 1200 quesn (same diff 1520D)a[j]-a[i]=j-i--->a[j]-j=a[i]-i (now we have to only found multiple val for a[i]-i for i(0 to n) will give number of pair))
//even[r]-odd[r]=even[l-1]-odd[l-1]--> now we have to find these pair
void themagician(){
in(n)
vl v;
vin(v,n);
...
debug(odd)
map<ll, ll>m;
m[0]++;//as intially both even ad odd are zero (so take case where we take sum from beginning like in case 1)
loop(n)
{
    m[even[i]-odd[i]]++;
    if( m[even[i]-odd[i]]>1)
