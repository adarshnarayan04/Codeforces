...
void themagician(){
    ...
    ll end1=-1,end2=-1;//storing end of last two process

    // no take care of start time it is alredy sortrd so next program will>=previous aklways

    loop(n)
    {
        if(v[i].fi<=end1&&v[i].fi<=end2)//means third program cmae when two programm already running
        {
            NO;
            return;
        }
        if(v[i].fi>end1)//2nd program come when 1st has ended(so 1 is running)
        {
            end1=v[i].se;
        }
        else{//2nd program come when 1 is already running so now (2 program is running)
            end2=v[i].se;
        }
    }
    YES;
