...
void dfs(ll vertex,ll par=0)
{
	...
	subtree_sum[vertex]+=vertex;//assuming value of that vertex has vertex number
	// if value given use =value[vertex]
    // can be written =vertex
    // but += written so that in any case vertex is computed later than eual will override the previos value
...
dfs(1);//so we can precompute for all tree
//rather than apllying dfs(v) for an particular vertex subtree 
// now for wach query it willatke O(1)
loop1n(n)
{
