...
void themagician(){
    ...
    loop1(n+1)
    {
        pref[i]=pref[i-1]+v[i-1];//v 0 based and pref is 1 based
    }
    ...
    while(q--)
    {
        ll l,u;
        cin>>l>>u;
        ll ind=lower_bound(pref.begin(),pref.end(),pref[l-1]+u)-pref.begin();//as we need more u secctions as it will give max
        //more than yaht lead to negitive
        if(ind==n+1)
        {
            ind--;
        }
        debug(ind,pref[ind])
        if(pref[ind]-pref[l-1]-(u+1)>=u-(pref[ind-1]-pref[l-1]))//means this ind has more differrence from u taken(u+1)as u+1 wll give act as 0 for greater than u
        //if both are less than u then 1st is -ive while 2nd pos so ind-- not exceuted
        ind--;
        ind=max(ind,l);
        cout<<ind<<" ";
