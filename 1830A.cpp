...
/*----------------------code start here -----------------------------*/
int dp[N];
void sol(ll val,ll ind,vl&v,map<ll,vpll>&m)//dfs
{
    // debug("YES")
    // debug(m[val])
       
            dp[val]=1;//mark as visted(here only not visted vertex come)
            

    loop(m[val].size())
    {
        if(dp[m[val][i].fi]!=-1)//child is visted
        {
            continue;
        ...
        {
           v[m[val][i].fi]=v[val];
            sol(m[val][i].fi,m[val][i].se,v,m);
        }
        else{
            v[m[val][i].fi]=v[val]+1;
            sol(m[val][i].fi,m[val][i].se,v,m);
        }
    }
