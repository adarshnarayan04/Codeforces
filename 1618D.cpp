...
for(auto &x:m)
{
	if(x.se>k)// if no of duplicate are greater than half (than extra one's(greater than half) cannot be cancelled else always can be cancelled)
	{
		sum+=(x.se-k);
		break;//there can be only one elemnt greater than half
	}
}
op(sum)
...
//approach 1(working)
// debug(sum)
// ll j=k+1,i=1; // as if v[i]=v[j] only when that number is present more than half number(based on above)
// while(k--)
// {
// 	if((v[j]/v[i]))sum++;
// 	i++;
