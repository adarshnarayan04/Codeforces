...
vl predown(n+1);
vl ans;
loop1(n+1){//1 based indexing
	debug(i)
	preup[i]=preup[i-1]+v[0][i];
	predown[i]=predown[i-1]+v[1][i];
}
...
loop1(n+1)
{
    //considering that alice go down in ith index
    //bob will move only up or only down so that it get max(imagine)
    //so we find max we get up and down
    //we do min as alice will alwys try to redice bob sum so bob min sum will be best chioce for alice to play(reverse thinking)
    //reverse thinking-- alicec doenot make move in which it got max ---->he sees which move make bob min then take that move
    ma=min({ma,max(preup[n]-preup[i],predown[i-1]-predown[0])});
