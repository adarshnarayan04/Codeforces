...
void themagician(){
    ...
    vl a,b;
    a.pb(0);
    b.pb(0);//1 based indexing
    vin(a,n);
    vin(b,n);
    ll min1=INF;
    ll sum=0;
    for(int i=m;i>0;i--)//we can go up only once then it stopped
    {
        if(i==m)
        {
            min1=a[m];
    ...
    debug(min1)
    ll ans=0;
    for (int i = m+1; i <=n; i++)//we can choose anyone to go uptil m
    {
        ans+=(min(a[i],b[i]));
    }
    op(ans+min1)
