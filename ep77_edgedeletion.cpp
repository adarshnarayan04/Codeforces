...
dfs(1);//assuming 1 to be root
ll ans=0;
for(int i=2;i<=n;i++)
{
	int part1=subtree_sum[i]; //as after removing edge ith vertex become the root of the subtree;
	int part2=subtree_sum[1]-part1; //as subtree_sum[1] will cointain sum of all as 1 is root of tree
	// so subtree of 1 is whole tree
	ans=max(ans,(1ll*part1*part2));// asked find max product
	//ans=max(ans,(1ll*part1*part2)%M); do not take modulo here as it will reduce its value here and in 
	// ans other value will be stored which is not maximum
	// take modulo at end
}
ans=ans%M;//taken at end so that result is not disorted
}
