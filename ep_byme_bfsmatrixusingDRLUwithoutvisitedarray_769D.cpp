...
int dp[N][N];//distance array
void themagician(){
	memset(dp,-1,sizeof(dp));
	vector<pair<ll, ll>> movement={//D L R U
		{1,0},{0,-1},{0,1},{-1,0}
	};
...
if(k&1)//we need even as for every up we need down and for every right we need left
{
	cout<<"IMPOSSIBLE";
	return;
}
...
while(!q.empty())
{
	...
	{
		ll chx=par.fi+mov.fi;
		ll chy=par.se+mov.se;
		if(!isvalid(chx,chy))continue;
		if(dp[chx][chy]==-1&&s[chx][chy]!='*') // means not visited
		{
			q.push({chx,chy});
			dp[chx][chy]=dp[par.fi][par.se]+1;
		}
	}
}
debug(source)
string mov="DLRU";
string ans;
ll x=source.fi,y=source.se;
while(k)
{
	bool ok=0;
	loop(4)//so that we can DLRU according
	{
		ll c=x+movement[i].fi;
		ll d=y+movement[i].se;
		if(isvalid(c,d)&&s[c][d]!='*'&&dp[c][d]!=-1&&dp[c][d]<k)
			//here k represent number of move left(we take 1 for moving th (c,d) therefor <k)
			//now we can go to index which distance from origin is less than k(so we can come back)
		{
			x=c;y=d;
			ans.pb(mov[i]);
			ok=1;
			k--;
			break;//only take the min
			
		}
		
	}
	if(!ok)//means no movement possible
	{
		cout<<"IMPOSSIBLE";
	    return;
	}
