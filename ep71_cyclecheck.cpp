...
bool dfs(ll vertex,ll par)
{
	...
	{
		if(vis[child]&&child==par) continue;//can also write if(vhild==par) continue; 
		//as parent is always visted before visiting child
		if(vis[child]) return true;//cycle is present  as we find  next element which is not parent and it is visted
		isloopexists |=dfs(child,vertex);//vertex if the parent for it
		// so if a single cycle is found it is true therefore use OR
        // so by backtracking parent also return true as isloopexists will remaian true as taking or
	}
	return isloopexists;
}
void themagician(){
...
bool loopexsist=false;
loop1n(n)//so that we apply dfs on all element (if graph is not connected)
//therefore we check all elemeent if taht elemnt is not visited we apply dfs on it
{
	if(vis[i]) continue;

	if(dfs(i,0))// for no parent for source so taken 0(not an vertex here as it is 1-n)
		//take -1(as 0 is not appliacble if vertex start from zero)
	{
		loopexsist=true;
		break;// if on any component the loop exsist it is true
	}
