...
int val[N][N];
bool vis[N][N];
int lev[N][N];//creating anarray as always pass by referennce (also easy to crete 2d array than 2d vector)
int n,m;// matrix size
void reset()
{
	loop(N)
...
};// to generate all movements is grid(like accesing the child of vertex) considering from origin(0,0)
...
int bfs()
{
	...
	debug(mx)
	queue<pair<int,int>> q;// 2 bfs in matrix so cointains coordinates
	loop(n)
	{
		loopj(m)
		{
			if(val[i][j]==mx)
			{
				q.push({i,j});//adding sorces(can be multiple) if mutilple max exsist
				lev[i][j]=0;
				vis[i][j]=1;
				debug(i)debug(j)
			}
		...
		while(!q.empty())
		{
			...
			{
				int child_x=movement.fi+v_x;
				int child_y=movement.se+v_y;
				if(!isvalid(child_x,child_y)) continue;
				if(vis[child_x][child_y]) continue;
				debug(child_x)debug(child_y)
