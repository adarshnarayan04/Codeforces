...
void themagician(){
    ...
    debug(x)
    auto check=[&](ll mid)//checking that it can complete all task in t time or not
    {
      ll sum=0;
      loop1(n+1)
      {
        ll one=min(mid,x[i]);//first taking all the one hour task(to maximise no of work done) he can complete in mid hour(so min(mid,on hour task))
        ll two=floor((mid-one)/2);//then taking number of two hour work that can be done in left time
        sum+=(one+two);//total taask done by i in mid hour
      }
      return sum>=m;
