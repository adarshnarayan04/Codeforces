...
void dfs(ll vertex,ll par=-1)//taking parent of root as -1
{
	p[vertex]=par;//storing parent of each vertex
...
vector<ll> path(ll vertex)//will return the path of the veretex
{
	vl ans;
	while(vertex!=-1)
	{
		ans.pb(vertex);
		vertex=p[vertex];// accessing the parent of the vertex	
	}
	reverse(all(ans));
	return ans;
}
...
ll x,y;// node of which to find lca to find
cin>>x>>y;
...
ll lca=-1;
ll size=min(path_x.size(),path_y.size());
loop(size)
{
