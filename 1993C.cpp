...
void themagician(){
    in(n)in(k)
     ...
     ll mi=v[n-1],ma=v[n-1]+k-1;// time to glow of first light
     //taken intersection of all light
     for (int i = 0; i <n-1; ++i)
     {
         ...
         if(div&1)//mean it off at v[n-1]--> finded time when it reglow
         {
         ...
         else//laredy glowing at v[n-1] --> finded time when it stop glowing
         {
            ll off=v[i]+(div+1)*k-1;//after one second it will be off
            debug(off,i)
            ma=min(ma,off);
