...
vl v;
vin(v,n);
//vl prefix(n+1);//1 based indexing
// prefix[0]=0;
//unordered_set<ll>s;
vl ins(n+1,0);
ins[1]=1;
...
while(q--)
{
    ...
    {
        if(v[r-1]!=v[ins[r]-1])// o based therefore -1
            //whwn same calue
        cout<<ins[r]<<" "<<r<<nl;
       else
       {
        cout<<ins[r]-1<<" "<<r<<nl;//ins[r] will give position of last insertion so ins[r]-1 will be diiferent element
       }
