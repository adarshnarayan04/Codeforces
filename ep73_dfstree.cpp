...
double acc = 1e-6;
vl g[N];
//bool vis[N]; No need as it is tree 
// so cycle case not possible only parent case is possible
vl depth(N);
vl height(N);
void dfs(int vertex,int par=0)
{/*take action veretx before enetring the vertex*/
	
	for(ll child:g[vertex])
	{
		/*take action on child before enetring the child node this sectopn goes upto dfs()*/
		if(child==par) continue;// do not use return (as it will end for loop)
		depth[child]=depth[vertex]+1;//vertex is parent here(do not use par as will give here)
		// par store parent of vertex not of child(as we call dfs(vertex,parent of vertex))
		//parent of child is vertex	
		
		/* this section end*/
		
		dfs(child,vertex);//as vertex will become the parent
		
		/*take action on child after exiting the child node this sectopn after dfs()*/
		
		height[vertex]=max(height[vertex],height[child]+1);//as now all vertex is computed as it is after dfs	
		
	}
	/*take action veretx after exiting the vertex*/
}
void themagician(){
in(n)

loop(n-1)//we take number of vertex so number of edges=n-1
{
	ll x,y;
	cin>>x>>y;
	g[x].pb(y);
