...
ll dijkstra(ll source,ll n,vl &s)
{//2d dijkstra (here {node,bike} pair is unique)
    //we take all possibilty as we can reach that vertex by various bike
	vector<vector<bool>>vis(N,vector<bool>(N,0));
	vector<vector<ll>>dist(N,vector<ll>(N,INF));//distance of all node source will be stored and the bike by which it has been reached
	set<pair< pair<ll, ll>,ll>>st;
	st.insert({{0,s[0]},source});//{wt,bike} set sort will sort the pair
	dist[source][s[0]]=0;
	...
	while(!st.empty())
	{
		auto node=*st.begin();
		int v=node.se;
		ll v_dist=node.fi.fi;
        ll bike=node.fi.se;
        ll new_bike=node.fi.se;
		st.erase(st.begin());//as processed so deleted from set
		if(vis[v][bike]) continue;
		vis[v][bike]=1;//as its child is going to be processed ,so mark as visited
        if(s[v]<bike)
        {
            new_bike=s[v];//means we will use this bike to travel to child
        }
		for(auto child:g[v])//{node,wt} in graph
		{
			ll child_v=child.fi;
			ll wt=child.se;
			if(dist[v][bike]+wt*new_bike<dist[child_v][new_bike])//we will ewach child by new vike therefore wt*new_bike and dist[child][newbike]
			{
				
                    dist[child_v][new_bike]=dist[v][bike]+new_bike*wt;
                    st.insert({{dist[child_v][new_bike],new_bike},child_v});
    ...
    return ans;//min to reach it by various bike
