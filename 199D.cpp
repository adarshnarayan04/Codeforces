...
loop(n)
{
	if(s[0][i]=='X')vis[0][i]=1;
	if(s[1][i]=='X')vis[1][i]=1;
}
debug(vis)
vector<vector<int>>lev(2,vector<int>(n,INF));
pair<int, int>source={0,0};

q.push(source);
lev[source.fi][source.se]=0;
vis[source.fi][source.se]=1;
while(!q.empty())
{
	...
	for(auto &mov:movements)
	{
		...
		if(childy>=n||!vis[childx][childy])
		{
			debug(mp(childx,childy))
			...
			if(lev[x][y]+1<=childy)//above the water
			{
