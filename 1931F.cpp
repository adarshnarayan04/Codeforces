...
/*----------------------code start here -----------------------------*/
/* se abhishjek saini video*/
void themagician(){
in(n)in(k)
vector<double>v(n+1);//storing there occurence order
...
loop(k)
{
	...
	else
	{
		fl(j,1,n)
		{
			...
			if(v[d[j]]==0)
			{
				v[d[j]]=v[d[j-1]]+1;//if j==1 d[j-1]=d[0]=0 which we have set
				continue;
			}
			
			if(j==1)//as we do d-1 and it start from 1
			{
				continue;
			}
			else if(v[d[j]]==v[d[j-1]])
			{
				v[d[j-1]]-=0.5;
			...
			{
				NO;
				flag=0;//do not use return so that can take next value that came
