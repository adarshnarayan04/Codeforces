...
bool isPerfectSq(ll n){ld root=sqrtl(n);return ceil(root)==root;}
...
NO;
//concept----> is that the ist factor=1 2nd=number itself so third factor should be
//1.prime(as if not prime then the number istelf have other factor which will also divide that number)
//2.should be squareroot of that number(i*j=x   then i,j are both factor -->let i,j be prime then also total will be 4 factors)
// i*i=x where i should be prime
}
