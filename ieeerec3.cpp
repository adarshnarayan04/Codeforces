...
ll binser(ll c)
{
	...
	while(hi-lo>1)
	{
		...
		mid=(hi+lo)/2;
		if(sum(mid)>c) hi=mid-1;
		else
		{
			lo=mid;
		}
	}
	if(sum(hi)<=c) return hi;
	else
		return lo;
}
ll sum2(ll i,ll h)
{
	ll x=i-h;
	return sum(h)+sum(h-1)-sum(h-1-x);
	// return((2*h-i-1)*i)/2;
}
ll binser2(ll c,ll h )
{
	ll lo=1;
	ll hi=2*h-1;
	ll mid;
	...
	{
		mid=(hi+lo)/2;
		if(sum2(mid,h)>=c) hi=mid;
		else
		{
			lo=mid+1;
		}
	}
	if(sum2(hi,h)>=c) return hi;
	else
		return lo;
}
void themagician(){
...
else if(sum(h)+sum(h-1)<=c)
{
	ll ans=2*h-1;
	op(ans);
}
else if(sum(h)<c&&sum(h)+sum(h-1)>c)
{
