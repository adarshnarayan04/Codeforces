...
#define vin(v,n) for(int i = 0; i < n; ++i){ld x;cin>>x;v.pb(x);}
...
/*----------------------code start here -----------------------------*/
 ld func(ld mid,vector<ld> &x,vector<ld> &t){
    ld ans=0;
    loop(x.size())
    {
        ld a=0;
...
void themagician(){
    ...
    while(hi-lo>1e-7)
    {
        ...
        ld y=(hi-lo)/4.0;
        ld mid=(hi+lo)/2.0;
        //as it bvehaviour may be diffewrnent on both side of mid so chenck both side

        ld mid1=lo+y;
        ld mid2=hi-y;

        ld ans1=func(mid1,x,t);
        ld ans2=func(mid2,x,t);
    ...
    if(func(lo,x,t)<func(hi,x,t))
    {
        cout<<fixed<<setprecision(7)<<lo<<nl;
    }
    else
    cout<<fixed<<setprecision(7)<<hi<<nl;
}
