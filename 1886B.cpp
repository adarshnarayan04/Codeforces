...
debug(px)
ld m1=max(dis(ax,ay,0,0),dis(px,py,ax,ay));
ld m2=max(dis(bx,by,0,0),dis(px,py,bx,by));
ld m3=max({dis(ax,ay,bx,by)/2,dis(ax,ay,0,0),dis(bx, by, px, py)});
ld m4=max({dis(ax, ay, bx, by) / 2, dis(ax, ay, px, py), dis(bx, by, 0, 0)});
cout<<fixed<<setprecision(10)<<min({m1,m2,m3,m4})<<nl;
}
