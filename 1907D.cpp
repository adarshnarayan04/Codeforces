...
/*----------------------code start here -----------------------------*/
bool check(ll k,vector<pair<ll, ll>>&v)
{
	ll l=0,r=0;//l--left r--right
	loop(v.size())
	{
		ll pos_l=l-k;//to indicate the range where we can go
		ll pos_r=r+k;
		ll valid_l=max(pos_l,v[i].fi);// taking intersection to get the possible range
		ll valid_r=min(pos_r,v[i].se);
		ll range=valid_r-valid_l;
		if(range<0)
		{
			return false;
		}
		else
		{
			l=valid_l;
			r=valid_r;
			//updating l and r position for next cases(as we can move from that psooible range for next case)
		}
...
if(check(lo,v)) cout<<lo<<nl;
else if(check(hi,v)) op(hi)
