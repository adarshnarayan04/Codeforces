...
for(auto &it:m)
{
    ...
    if(b==y)b=0;
    debug2(a,b)
    if(a<it.fi.fi||(a==it.fi.fi&&b<it.fi.se))//so that search only for element greater or equal to it so that duplicates is removed
    {
        break;
    }
    ...
    if(a==it.fi.fi&&b==it.fi.se)
    {
        if(it.se>1)
            count+=(((it.se)*(it.se-1))/2);
    }
    else
    {
        if(m.find({a,b})!=m.end())
