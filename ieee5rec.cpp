...
{//2d dijkstra (here {node,bike} pair is unique)
	...
	vector<vector<ll>>dist(N,vector<ll>(N,INF));//distance of all node source will be stored
	...
	while(!st.empty())
	{
		...
		for(auto child:g[v])//{node,wt} in graph
		{
			ll child_v=child.fi;
			ll wt=child.se;
			if(dist[v][bike]+wt*new_bike<dist[child_v][new_bike])
			{
