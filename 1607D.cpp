...
//assuming at last when permuaution is made it is like BBBBRRRR as all blue take min place and red take max palce
// iff it BBRBBRRB then this blue and red which are noot in place can swap their pklace as B can decresae it vlaue and red can increase
// then become BBBBRRR

//now we are checking for number 1 to n(l.size()+r.size()=n) does that red or blue esiist
loop(l.size())
{
    if(l[i]<i+1)//i repeensnet the number of permutation therefore plus one
    {
        NO;//means blue is present below this num so it blue can take that value;
        // thus it canot form an permuatution as this place will remain empty
        //last l.size() are blue and r.size() red if it form an permutation
        return;
    }
}
loop(r.size())
{
    if(r[i]>n-i)//similar with red
    {
        NO;
        return;
    }
