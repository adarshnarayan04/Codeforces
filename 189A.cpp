...
ll func(ll left,ll taken)
{
	...
	if(dp[left][taken]!=-1) return dp[left][taken];
	ll ans=0;
	for(auto &x:v)
	{
		if(left-x>=0)
		{
			ans=max(ans,func(left-x,taken+1));
		}
		
	}
	return dp[left][taken]=ans;//all dp store the smae result(supreme parent(func(n,0)) result-->as it will  max total coin tken by supreme parent )
