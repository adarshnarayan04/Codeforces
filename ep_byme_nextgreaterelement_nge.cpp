...
vl v;
vin(v,n);
stack<ll>st;//storing nge position
vl ans(n);
...
{   
	...
	// else
		...
		while(!st.empty()&&v[i]>=v[st.top()])//if smaller or equal number than st.top() keep removing
		{
			st.pop();
		}
		if(st.size()==0)
		...
		else//if number is smaller than st.top() then insert
		{
			ans[i]=st.top();
			st.push(i);
		}
