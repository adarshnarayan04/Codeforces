...
/*----------------------code start here -----------------------------*/
bool isCyclic(vector<vector<ll>> &adj){
	int V = adj.size();
...
while(k--)
{
	...
	fl(i,2,n)//as 0 index of no use
	{
		g[a[i-1]].pb(a[i]);//it is diresrced graph
	}
}
//if cycle exsist in it --->measn not possible
//as if data is correct graph is always moving downwards so cycle can not exsist
//due to incorrect data graph will move upward forming an cycle
// if cycle is formed we can order them (therfore become NO)

//inp uswe cycle check for directed draph
