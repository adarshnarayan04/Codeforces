...
/*----------------------code start here -----------------------------*/
ll dp[100000+10][3];
ll maxhei(vl &a,vl &b,ll ind,ll k)
{
	...
	if(k==0)
	{
		ma=max(maxhei(a,b,ind+1,1)+a[ind],ma);
		ma=max(maxhei(a,b,ind+1,2)+b[ind],ma);
		
		ma=max(maxhei(a,b,ind+1,0),ma);
	}
	else
	{
		if(k==2)
